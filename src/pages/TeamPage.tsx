      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {TEAM_MEMBERS.map((member, idx) => (
            <div
              key={idx}
              className="bg-white group p-8 rounded-3xl shadow-sm border border-slate-100 hover-lift text-center"
            >
              {/* Avatar */}
              ...
              {/* Info */}
              <h3 className="text-xl font-bold text-slate-900 group-hover:text-ugass-green transition-colors">
                {member.name}
              </h3>

              <div className="text-ugass-gold text-sm font-bold uppercase tracking-wider mb-4">
                {member.role}
              </div>
